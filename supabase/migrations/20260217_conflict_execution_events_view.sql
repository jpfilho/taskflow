-- ============================================
-- VIEW: v_conflict_execution_events
-- Eventos de EXECUÇÃO por (executor, dia, local, tarefa) para detecção de conflito no backend.
-- Regras: só EXECUÇÃO; status CANC/RPGR/REPR/RPAR excluídos; prioridade:
--   executor_periods da tarefa → executor_periods do pai → filhos (mesmo executor) → gantt_segments.
-- ============================================
...
-- Status a excluir (CANC, RPGR, etc.) — usa apenas tasks.status
...
WITH
  ...
  -- 1) Eventos de executor_periods da própria tarefa (EXECUCAO). Subtarefa NÃO entra aqui se o pai tiver executor_periods para este executor.
  ev_ep_tarefa AS (
    SELECT
      ep.executor_id,
  ...
  -- 2) Eventos de executor_periods do PAI (subtarefa usa período do pai)
  ev_ep_pai AS (
    SELECT
      ep.executor_id,
      e.nome AS executor_nome,
  ...
  -- 3) Eventos de gantt_segments (só para tarefas SEM executor_periods para esse executor e SEM filhos)
  ev_gs AS (
    SELECT
      te.executor_id,
      e.nome AS executor_nome,
  ...
  -- 4) Eventos de tarefa PAI (só quando algum filho do mesmo executor tem evento naquele dia)
  ev_pai_por_filho AS (
...
'Eventos de EXECUÇÃO por (executor, dia, local, tarefa) para detecção de conflito. Conflito = mesmo executor e dia com mais de um location_key distinto.';

-- ============================================
-- VIEW: v_conflict_por_dia_executor
-- Por (executor_id, day): has_conflict e descrições para tooltip.
-- ============================================
...
'Conflito por (executor, dia): has_conflict true quando mais de um local no mesmo dia; descriptions para tooltip.';
