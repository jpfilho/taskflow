-- ============================================
-- VIEWs: conflito de FROTA (mesmo padrão do conflito de executor)
-- v_conflict_execution_events_frota: eventos de EXECUÇÃO por (frota_id, dia, local, tarefa).
-- v_conflict_por_dia_frota: has_conflict e descriptions por (frota_id, day).
-- Regras: só EXECUÇÃO; mesmos status e tipo excluídos (CANC, RPGR, ADMIN, ADM, REUNIAO).
-- ============================================
...
WITH
  ...
  -- 1) Eventos de frota_periods (EXECUCAO)
  ev_fp AS (
    SELECT
      fp.frota_id,
  ...
  -- 2) Eventos de gantt_segments quando a tarefa tem tasks_frotas mas não tem frota_periods para essa frota
  ev_gs_frota AS (
    SELECT
      tf.frota_id,
  ...
  -- 3) Fallback: datas gerais da tarefa quando não há segmentos nem frota_periods
  ev_task_base AS (
    SELECT
      tf.frota_id,
      COALESCE(f.nome || ' - ' || f.placa, f.nome, ''::text) AS frota_nome,
...
'Eventos de EXECUÇÃO por (frota, dia, local, tarefa) para detecção de conflito de frota. Conflito = mesma frota e dia com mais de um local distinto. Exclui CANC/RPGR e tipo ADMIN/ADM/REUNIAO.';

-- ============================================
-- VIEW: v_conflict_por_dia_frota
-- Por (frota_id, day): has_conflict e descriptions para tooltip.
-- ============================================
...
'Conflito por (frota, dia): has_conflict true quando mais de um local no mesmo dia; descriptions para tooltip.';
